    ...
    <div className="flex flex-wrap items-center gap-4" role="group" aria-label="Filter tasks">
      <Select<{ id: string }>
        ...
        onSelectionChange={(key) => {
          setFilter('status', key === null ? undefined : (String(key) as Status))
        }}
      >
      ...
      <Select<{ id: string }>
        ...
        selectedKey={filters.pointEstimate ?? null}
        onSelectionChange={(key) => {
          setFilter('pointEstimate', key === null ? undefined : (String(key) as PointEstimate))
        }}
      >
      ...
      <Select<{ id: string }>
        ...
        selectedKey={filters.ownerId ?? null}
        onSelectionChange={(key) => {
          setFilter('ownerId', key === null ? undefined : String(key))
        }}
      >
        {(item) => (
          <Item key={item.id}>
