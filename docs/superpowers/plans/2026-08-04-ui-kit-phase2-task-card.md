...
- **Baseline:** on this branch's actual base (`origin/dev` @ `b7326b2`, which includes PR #13 /
  Phase 0+1), `npm run gate` is **fully green — 0 failures**, confirmed 2026-08-04. This
  supersedes any mention elsewhere (`UI_KIT_MIGRATION_PLAN.md`, this plan's own earlier drafts) of
  "2 pre-existing failures" — those were fixed when PR #13 merged into `dev`. The bar for this
  plan is 0 failures, not 2 tolerated ones.
- **Gate discipline across tasks:** `npx tsc --noEmit`, lint, and format must be green at the end
  of every task from Task 2 onward. The full test suite (and therefore full `npm run gate`) is
  allowed to show failures **only** in test files a later, explicitly-numbered task in this plan
  is about to fix — Task 2 (below) knowingly leaves `board.test.tsx`, `update-delete-task.test.tsx`,
  and possibly `search-filter.test.tsx` red, because Tasks 3-5 fix exactly those files next, in
  the very next dispatches. A task reviewer seeing red tests **outside** that named set, or a
  typecheck/lint/format failure at any point, is a real finding, not an expected one. By the end
  of Task 5, `npm run gate` must be fully green again with 0 failures, and must stay that way
  through Tasks 6-7.
...
- Branch fresh off `dev` (per this repo's branch layout), not off `feat/consume-ui-kit`. (Already
  done: branch `feat/ui-kit-task-card` off `origin/dev` @ `b7326b2`, in an isolated worktree.)
...
TaskTableRowProps`, both exported from `to-kit-props.ts`. Task 2 calls these directly.
...
- [ ] **Step 5: Typecheck and commit — do NOT delete the old `task-card.tsx`/`due-date-badge.tsx` yet**

`board-column.tsx` still imports the old `TaskCard` component (Task 2 rewrites it and retires
these files together, since deleting them now would leave `board-column.tsx` failing to typecheck
at this task's own commit). Leave the old files in place, unused by anything new, harmless until
Task 2 removes their only caller.

Run: `npx tsc --noEmit`
Expected: no errors anywhere (the new adapter compiles; nothing yet references it, and nothing
was removed).
...
### Task 2: Rewire board rendering onto the kit, and move Delete into the edit dialog
...
- Delete: `src/features/board/task-card/task-card.tsx`,
  `src/features/board/task-card/task-card.test.tsx`,
  `src/features/board/task-card/due-date-badge.tsx`

**Why this is one task, not four:** `board-column.tsx` (grid), `board.tsx`'s list branch, and
`board-page.tsx`'s wiring to both `Board` and `TaskFormDialog` share two props
(`onDeleteTask`/`onDelete`) that only make sense removed and added together — splitting this into
separately-reviewed tasks would leave `npx tsc --noEmit` broken at every intermediate task
boundary, violating the Global Constraint that typecheck/lint/format stay green from this task
onward. This task's _tests_ (not typecheck) are the one place a known, temporary red is expected —
see the Global Constraints note above and Step 9 below.

**Interfaces:**

- Consumes: `toKitCardProps`/`toKitTableRowProps` from Task 1.
- Produces: `BoardColumn({ status, tasks, now, className, onEditTask }: BoardColumnProps)` — same
  name, `onDeleteTask`/`itemLayout` props removed (list view is now a wholly different kit
  component, not a layout flag on the same one). `TaskFormDialogProps` gains
  `onDelete?: () => void`, rendered as a "Delete task" button only when `mode === 'edit'` and
...
- [ ] **Step 2: Rewrite `board.tsx`'s grid branch and shared `now` handling**

Resolve `now` once at the top of `Board` (the adapter takes a required `Date`, not an optional
one — a deliberate change from the old `TaskCard`'s own `now = new Date()` default, since the
adapter is meant to be a pure function with no hidden clock read of its own). Drop
`onDeleteTask` from `BoardProps` entirely (Delete no longer has a card-level trigger):
...
export function Board({ tasks, view, now, onEditTask }: BoardProps) {
  ...
  // ...list branch rewritten in Step 3 below...

  return (
    // ...grid branch: <BoardColumn> calls pass now={effectiveNow}, no onDeleteTask, no itemLayout
  )
}
```

- [ ] **Step 3: Replace `board.tsx`'s `view === 'list'` branch with one kit `TaskTable`**

The kit's `TaskTable` groups every status internally (one `<table>` per group), replacing the old
per-status `<BoardColumn itemLayout="row">` loop with a single component call.
`TaskTableRowProps.index` needs a **1-based** number per group (per this plan's kit-facts section)
to produce the documented "01", "02" numbering:
...
// inside Board, before the grid branch's return:
if (view === 'list') {
  /*
...
- [ ] **Step 4: Typecheck `board.tsx`/`board-column.tsx` in isolation**

Run: `npx tsc --noEmit`
Expected: errors remaining only in `board-page.tsx` (fixed in Step 8 below) and in test files
(fixed in Tasks 3-5). No errors in `board.tsx` or `board-column.tsx` themselves.

- [ ] **Step 5: Commit the rendering swap**

```bash
git add src/features/board/board-column.tsx src/features/board/board.tsx
git commit -m "feat: render board columns and list view through @ravn/ui-kit's TaskListView/TaskTable"
```

- [ ] **Step 6: Write the failing test for the Delete button in `TaskFormDialog`**
...
convention rather than introducing a second one. Run
`npx vitest run src/features/board/task-form-dialog.test.tsx -t 'delete'` first and confirm FAIL
(no such prop/button exists yet).

- [ ] **Step 7: Add the `onDelete` prop and button to `TaskFormDialog`**

Add `onDelete?: () => void` to `TaskFormDialogProps`, destructure it, and render it before the
Cancel/Save pair, edit-mode-only:
...
text button instead of inventing one. Run
`npx vitest run src/features/board/task-form-dialog.test.tsx` and confirm PASS, including the 3
new cases and all pre-existing ones.

- [ ] **Step 8: Rewire `board-page.tsx`**

- Remove `onDeleteTask` from the `<Board>` call (Board no longer has that prop after Step 2).
...
- [ ] **Step 9: Full typecheck, then delete the old `TaskCard`/`DueDateBadge`, then commit**

Run: `npx tsc --noEmit`
Expected: **zero errors anywhere** — this is the point where every production file this task
touches is internally consistent. (Test files may still fail when run, per the Global Constraints
note — that's expected and owned by Tasks 3-5, not this step.)

Confirm nothing else references the old component, then remove it:

```bash
grep -rn "task-card/task-card'\|DueDateBadge" src --include="*.ts" --include="*.tsx"
```

Expected: only the 3 files being deleted show up (everything that rendered a task now goes through
`board-column.tsx`/`board.tsx`, rewired above). Delete them and commit:
...
- [ ] **Step 10: Run the full test suite and confirm the expected (and only the expected) red files**

Run: `npx vitest run`
Expected: failures confined to `board.test.tsx`, `update-delete-task.test.tsx`, and possibly
`search-filter.test.tsx` (fixed in Tasks 3-5 next). Any failure outside those three files is a
real regression this task introduced — stop and fix it before calling this task done.

---

### Task 3: Rewrite `board.test.tsx`
...
- [ ] **Step 1: Confirm the per-status heading assertions (grid view) still pass unchanged**
...
### Task 4: Rewrite `update-delete-task.test.tsx`
...
Verify in a real browser too (Task 6) — React Aria's restore-focus target here is whatever the kit
...
(`editDialog.close()`) the moment `deleteDialog.open()` runs (Task 2's wiring) — so cancelling the
...
### Task 5: Check `search-filter.test.tsx` and any other remaining references
...
If a test fails because of the empty-state copy change (Task 3, Step 2) or anything else touched
by this phase, fix it the same way Task 3 did — match the kit's actual rendered text/structure,
...
Expected: only files already handled in Tasks 3/4 (and this one) show up. Fix any surprise.

- [ ] **Step 4: Run the full suite and confirm gate is fully green again**

Run: `npm run gate`
Expected: fully green, 0 failures — this is the point where the Global Constraints' temporary
test-red allowance ends. Everything from here on must be fully green.
...
where focus actually lands — this is exactly the number Step 6 of Task 4 needs before it can write
...
is one unlabeled clickable region. Record the actual observed name in the gap writeup (Task 7) —
...
### Task 7: Document this phase's kit gaps and update the persistent plan
...
Task 6 observed that this plan could only predict from reading the bundle):
...
3. **The whole card is an unlabeled `role="button"` `<div>`, not an `<article>`,** when `onClick`
   ...
   Record what Task 6 Step 4 actually observed here.
...
Run: `git diff origin/dev --stat` to confirm no domain type (`Task`, `User`) was reshaped and
nothing on the "stays permanently" list (`Menu`, `Select`/`MultiSelect`/`ListBox`/`Popover`,
`ToastProvider`, `EmptyState`, `ErrorBoundary`, icon set, `DeleteTaskDialog`) was touched —
`DeleteTaskDialog` itself should show zero diff; only its callers changed.
